...
class Drawing:
    ...
    def health(self, player):
        ...
        font = pygame.font.Font(None, 36)
        text = font.render(f'Health: {int(health)}', True, DARKORANGE)
        self.sc.blit(text, (10, 40))
    ...
    def win(self):
        ...
        self.sc.blit(render, (rect.centerx - 460, rect.centery - 90))
        pygame.display.flip()
        self.clock.tick(15)

    def lose(self):
        render = self.font_win.render('YOU DEAD!!', 1, (randrange(40, 120), 0, 0))
        rect = pygame.Rect(0, 0, 1000, 300)
        rect.center = HALF_WIDTH, HALF_HEIGHT
        pygame.draw.rect(self.sc, BLACK, rect, border_radius=50)
        self.sc.blit(render, (rect.centerx - 460, rect.centery - 90))
    ...
    def menu(self):
        ...
        label_font = pygame.font.Font('font/font1.otf', 70)
        start = button_font.render('ENTER', 1, pygame.Color('lightgray'))
        ...
        while self.menu_trigger:
            ...
            color = randrange(40)
            label = label_font.render('HE KNOWS WHERE YOU ARE.', 1, (color, color, color))
            self.sc.blit(label, (WIDTH // 2 - 370, 200))
            color = randrange(40)
            label = label_font.render('FIND A SOUL', 1, (color, color, color))
            self.sc.blit(label, (WIDTH // 2 - 190, 260))

            mouse_pos = pygame.mouse.get_pos()
            mouse_click = pygame.mouse.get_pressed()
            if button_start.collidepoint(mouse_pos):
                pygame.draw.rect(self.sc, DARKGRAY, button_start, border_radius=25)
                self.sc.blit(start, (button_start.centerx - 130, button_start.centery - 40))
                if mouse_click[0]:
